	...
	if(find_archetype(string)!=NULL) { 
		...
		LOG(llevBug,"BUG: Formula %s has no artifact\n",formula->title);
	     ...
	     if(strcmp(formula->title,"NONE"))
             ...
             LOG(llevInfo,"%-30s(%d) bookchance %3d  ",buf,formula->index,
		formula->chance);
             LOG(llevInfo,"\n");
		...
		int nval=0,tval=0;
		  ...
		  if(nval!=0) LOG(llevInfo,","); 
                  LOG(llevInfo,"%s(%d)",next->name,(nval=strtoint(next->name)));
		  tval += nval;
		}
                LOG(llevInfo,"\n");
		if(tval!=formula->index) LOG(llevInfo,"WARNING:ingredient list and formula values not equal.\n");
             }  
	  }
	} else 

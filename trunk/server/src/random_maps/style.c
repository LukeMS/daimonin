...
{
    if (the_entry->d_name[0] == '.')
        return 0;
    if (strstr(the_entry->d_name, "CVS"))
        return 0;
    return 1;
}
...
mapstruct * load_style_map(char *style_name)
{
    ...
    style_map = load_map(NULL, style_name, MAP_STATUS_STYLE, NULL);
    ...
    if (style_map == NULL)  /* maybe we were given a directory! */
    {
        ...
        else
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                {
                    /* since there isn't a sequence, */
                    int q;
                    /*pick one at random to recurse */
                    style_map = find_style(style_file_path, namelist[RANDOM() % n]->d_name, difficulty);
                    for (q = 0; q < n; q++)
                        free(namelist[q]);
                    free(namelist);
                    return style_map;

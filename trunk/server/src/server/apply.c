...
static int apply_altar (object *altar, object *sacrifice, object *originator)
{
  ...
  if (operate_altar (altar, &sacrifice))
  {
    ...
    if (altar->stats.sp != -1)
    {
      new_draw_info_format (NDI_WHITE, 0, originator, "The altar casts %s.",
...
extern void do_learn_spell (object *op, int spell, int special_prayer)
{
    ...
    if (special_prayer)
      insert_special_prayer_mark (op, spell);
        ...
        new_draw_info_format (NDI_UNIQUE, 0, op, 
            "You have learned the spell %s!", spells[spell].name);
}
			...
			*/
			...
			if(!QUERY_FLAG(tmp, FLAG_CURSED)&&!QUERY_FLAG(tmp, FLAG_DAMNED)) 
			{
				...
				if (!is_dragon_pl(op))
				{
					...
					else /* tasted "good" if food - tasted "terrible" for flesh for all non dragon players */
						sprintf(buf,"The %s tasted %s",tmp->name, tmp->type==FLESH?"terrible!":"good.");
				}
				else /* eating message for dragon players - they like it bloody */
...
void eat_special_food(object *who, object *food) 
{
  ...
  if(food->stats.Pow ||food->stats.Str || food->stats.Dex || food->stats.Con 
					|| food->stats.Int || food->stats.Wis || food->stats.Cha)
...
{ /* wear/wield */
    ...
    case HORN:
      ...
      if(who->type==PLAYER) {
        ...
        switch (op->type) {
          case ROD:
          case HORN:
          case WAND: CLEAR_FLAG (who, FLAG_READY_RANGE); break;
   ...
   */
    ...
    {
        if((tmp->type==op->type ||
                (tmp_flag &&(tmp->type==WAND|| tmp->type==ROD||tmp->type==HORN) ) )
  ...
  case BOW:
    ...
    if(who->type==PLAYER) {
      ...
      switch (op->type) {
        case ROD:
        case HORN:
        case WAND: SET_FLAG (who, FLAG_READY_RANGE); break;
...
void apply_player_light_refill(object *who, object *op)
{
    ...
    {
        new_draw_info_format(NDI_UNIQUE, 0, who,
                   "Mark a light source first you want refill.");
        return;
    }
    ...
    {
        new_draw_info_format(NDI_UNIQUE, 0, who,
            "You can't refill the %s with the %s.", query_name(item),query_name(op));
        return;
    }
    ...
    {
        new_draw_info_format(NDI_UNIQUE, 0, who,
            "The %s is full and can't be refilled.", query_name(item));
        return;
    }
    ...
    if(op->stats.food <= tmp)
    {
        ...
        new_draw_info_format(NDI_UNIQUE, 0, who,
            "You refill the %s with %d units %s.", query_name(item),op->stats.food, query_name(op));
        decrease_ob(op);
    }
    else
    {
        ...
        if(op->glow_radius <0)
        {
            if(op->last_eat) /* we have a non permanent source */
            {
                if(!op->stats.food) /* if not permanent, this is "filled" counter */
                {
                     ...
                     */
                    new_draw_info_format(NDI_UNIQUE, 0, who,
                        "You must first refill or recharge the %s.",
                        query_name(op));
                    return;
                }                    
            }
...
void apply_changes_to_player(object *pl, object *change) {
	...
	case CLASS: {
		...
		set_attr_value(stats,i,(sint8) stat);
	    }

...
{
    "vibrates briefly", "produces a cloud of steam", "emits bright flames", "pours forth heavy black smoke",
    "emits sparks", "shoots out small flames", "whines painfully", "hiccups loudly", "wheezes", "burps", "shakes",
    "rattles", "makes chugging sounds", "smokes heavily for a while"
};
...
char * cauldron_sound(void)
{
    ...
    return cauldron_effect[random_roll(0, size - 1)];
}
    ...
    if ((fl = get_formulalist(numb)))
    {
        ...
        if ((pl->gmaster_mode & GMASTER_MODE_SA))
        {
            ...
            if (rp)
            {
                ...
                LOG(llevDebug, "WIZ couldnt find formula for ingredients.\n");
            return;
        } /* End of WIZ alchemy */
        ...
        if (rp)  /* if we found a recipe */
        {
            ...
            if ((item = attempt_recipe(caster, cauldron, ability, rp, formula / rp->index)) != NULL)
            {
                ...
                success_chance = ((float)
                                  (15 * ability)
                                / (float)
                                  (15 * ability + numb * item->level * (numb + item->level + formula / rp->index)));
                if (ave_chance == 0)
                    ave_chance = 1;
                ...
                if (ave_chance > rp->chance)
                    ...
                    success_chance = 1.0f
                                   - ((1.0f - success_chance) * ((float) rp->chance + ave_chance)
                                    / (2.0f * (float) rp->chance));
                ...
                /* roll the dice */
                if ((float) (random_roll(0, 99)) <= 100.0 * success_chance)
                {
...
void adjust_product(object *item, int lvl, int yield)
{
    ...
    if (item->nrof)
    {
        ...
        nrof = (int)
               ((1.0f - 1.0f / ((float) lvl / 10.0f + 1.0f)) * (float)
                (random_roll(0, yield - 1) + (float) random_roll(0, yield - 1) + (float) random_roll(0, yield - 1))
              + 1.0f);
        if (nrof > yield)
            nrof = yield;
        item->nrof = nrof;
...
void alchemy_failure_effect(object *op, object *cauldron, recipe *rp, int danger)
{
    ...
    else if (level < 40)
    {
        ...
        if ((tmp = attempt_recipe(op, cauldron, 1, rp, -1)))
        {
            ...
            {
                /* thats a bug: sp in POTION is spell id not mana */
                if (tmp->stats.sp && random_roll(0, 1)) /* drains magic */
                ...
                if (random_roll(0, 1))
                {
                    /* poisonous */
                    tmp->type = FOOD;
                    tmp->stats.hp = random_roll(0, 149);
                }
            }
    ...
    } if (level == 40)
    {
      ...
      else if (level < 100)
      {
          ...
          if (!QUERY_FLAG(cauldron, FLAG_CURSED))
              ...
              cauldron->magic--;
               cauldron->magic -= random_roll(0, 4);
          if (random_roll(0, 1))
          {
              remove_contents(cauldron->inv, NULL);
      ...
      else if (level == 151)
      {
          ...
          {
              generate_artifact(tmp, random_roll(1, op->level / 2 + 1) + 1, 0, 99);

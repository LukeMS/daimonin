...
void set_dumpmonA() {settings.dumpvalues=11; }
...
static void load_settings()
{
	...
	if (!strcasecmp(buf,"metaserver_notification")) {
	    ...
	    if (size < 0)
		LOG(llevBug, "BUG: load_settings: worldmapstartx must be at least"
		    "0, %d is invalid\n", size);
	    else
	    ...
	    if (size < 0)
		LOG(llevBug, "BUG: load_settings: worldmapstarty must be at least"
		    "0, %d is invalid\n", size);
	    else
	    ...
	    if (size < 1)
		LOG(llevBug, "BUG: load_settings: worldmaptilesx must be greater"
		    "than 1, %d is invalid\n", size);
	    else
	    ...
	    if (size < 1)
		LOG(llevBug, "BUG: load_settings: worldmaptilesy must be greater"
		    "than 1, %d is invalid\n", size);
	    else
	    ...
	    if (size < 1)
		LOG(llevBug, "BUG: load_settings: worldmaptilesizex must be"
	    ...
	    if (size < 1)
		LOG(llevBug, "BUG: load_settings: worldmaptilesizey must be"
		    "greater than 1, %d is invalid\n", size);
	    else
	    ...
	    if (lev < 0)
		LOG(llevBug, "BUG: load_settings: dynamiclevel must be"
...
void init_startup() {
  ...
  if (forbid_play()) { /* Maybe showing highscore should be allowed? */
      LOG(llevSystem, "Daimonin: Playing not allowed.\n");
      exit(-1);
  }
}
...
#ifdef PERM_FILE
  LOG(llevInfo,"Perm file:\t%s/%s\n",settings.localdir,PERM_FILE);
#endif
#ifdef SHUTDOWN_FILE

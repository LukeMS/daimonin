...
void polymorph_living(object *op)
{
     ...
     */
    if (op->level >= 20
     || random_roll(1, 20, op, PREFER_HIGH) + op->resist[ATNR_MAGIC] / 10 > savethrow[op->level]
...
{
    /* Not unique */
    new_draw_info_format(NDI_WHITE, 0, who, "%s%s glows red, melts and evaporates!", op->nrof ? "" : "The ",
                         query_name(op));

...
int summon_pet(object *op, int dir, SpellTypeFrom item)
{
    ...
    */
    ...
    if (!dir)
        dir = find_free_spot(at, NULL, op->map, op->x, op->y, INS_WITHIN_LOS, 1, SIZEOFFREE - 1);
...
int summon_cult_monsters(object *op, int old_dir)
{
    ...
    do
    {
        ...
        if (!dir)
            dir = find_free_spot(mon->arch, mon, op->map, op->x, op->y, INS_WITHIN_LOS, 1, SIZEOFFREE - 1);
...
int animate_weapon(object *op, object *caster, int dir, archetype *at, int spellnum)
{
    ...
    if (!dir)
        dir = find_free_spot(NULL, NULL, op->map, op->x, op->y, 0, 1, SIZEOFFREE1);

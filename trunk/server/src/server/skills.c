    ...
    */
    ...
    if (!dir || (eff_str <= 1) || wall(op->map, op->x + freearr_x[dir], op->y + freearr_y[dir]))
    {
        ...
        if (op->type == PLAYER)
        {
            if (eff_str <= 1)
            {
                new_draw_info_format(NDI_UNIQUE, 0, op, "Your load is so heavy you drop %s to the ground.",
                                     query_name(throw_ob));
            }
            else if (!dir)
            {
                new_draw_info_format(NDI_UNIQUE, 0, op, "You drop %s at the ground.", query_name(throw_ob));
            }
            else

...
mapstruct *enter_map_by_name(object *op, const char *path, const char *src_path, int x, int y, int flags)
{
    ...
    */
    ...
    newmap = ready_map_name(path, src_path, MAP_STATUS_TYPE(flags));
    ...
    if (!newmap) /* map don't exists, fallback to savebed and/or emergency when possible */
    {
        ...
        if(op->type == PLAYER && CONTR(op))
        {
            ...
            newmap = ready_map_name(CONTR(op)->savebed_map, CONTR(op)->orig_savebed_map, CONTR(op)->bed_status);
            x = CONTR(op)->bed_x;
            y = CONTR(op)->bed_y;
            ...
            if(!newmap)
            {
                ...
                newmap = ready_map_name(shstr_cons.emergency_mappath, shstr_cons.emergency_mappath, MAP_STATUS_MULTI);
...
int enter_map_by_exit(object *op, object *exit_ob)
{
    ...
    */
    ...
    newmap = ready_map_name( file_path, exit_ob->race, mstatus);

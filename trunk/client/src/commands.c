...
void AnimCmd(unsigned char *data, int len)
{
        ...
        animations[anum].faces =
                _malloc(sizeof(uint16)*animations[anum].num_animations, "AnimCmd(): facenum buf");
...
void StatsCmd(unsigned char *data, int len)
{
						...
						break;
                               ...
                               case CS_STAT_HP:
                                ...
                                case CS_STAT_SKILLEXP_WISDOM:
                                    cpl.stats.skill_exp[(c-CS_STAT_SKILLEXP_START)/2] =
        ...
        {
                fprintf(stderr,
                "got stats overflow, processed %d bytes out of %d\n",
                i, len);
        }
}
...
void UpdateItemCmd(unsigned char *data, int len)
{
        ...
        *name='\0';
        ...
        if (sendflags & UPD_LOCATION)
        {
                loc=GetInt_String(data+pos);
                env=locate_item(loc);
                if (!env) fprintf(stderr,
                "UpdateItemCmd: unknown object tag (%d) for new location\n",
                loc);
                pos+=4;
        }
        ...
        {
                fprintf(stderr,
                "UpdateItemCmd: Overread buffer: %d > %d\n", pos,len);
                return;
        }

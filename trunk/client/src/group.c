...
void widget_show_group(int x, int y)
{
    ...
    StringBlt(ScreenSurface, &font_tiny_out, "Group:", x +50, y+2 , COLOR_HGOLD, NULL, NULL);


    if (global_group_status < GROUP_INVITE)
    {
        StringBlt(ScreenSurface, &font_tiny_out, "type '/help group' for info", x+13, y+13, COLOR_WHITE, NULL, NULL);
        return;
    }
    ...
    else /* status: GROUP_MEMBER */
    {
        ...
        for (s = 0; s < GROUP_MAX_MEMBER; s++)
        {
            ...
            {
                sprite_blt(Bitmaps[BITMAP_GROUP], x + group_pos[s][0] + 2, y + group_pos[s][1] + 1, NULL, NULL);
                StringBlt(ScreenSurface, &font_small, group[s].name, x + group_pos[s][0] + 33, y + group_pos[s][1] + 1,COLOR_DEFAULT, NULL, NULL);
                sprintf(buf, "%3d", group[s].level);
                StringBlt(ScreenSurface, &font_tiny_out, buf, x + group_pos[s][0] + 8, y + group_pos[s][1], COLOR_DEFAULT, NULL, NULL);
...
void show_group(int x, int y)
{
    ...
    */
    ...
    if (global_group_status < GROUP_INVITE)
    {
        StringBlt(ScreenSurface, &font_tiny_out, "type '/help group' for info", 40, Screensize.yoff+585, COLOR_WHITE, NULL, NULL);
        return;
    }

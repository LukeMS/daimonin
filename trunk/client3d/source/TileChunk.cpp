...
//#define NO_LOW_REDERING 1

///================================================================================================
/// Init static elements.
///================================================================================================
TileManager   *TileChunk::m_TileManagerPtr = 0;
AxisAlignedBox *TileChunk::m_bounds = 0;
...
void TileChunk::Attach(short quality)
{
    ...
    else
    {
        ...
        if (m_Water_entity_low)
            m_Water->attachObject(m_Water_entity_low);
    }
    ...
    for (short a = x; a < x+ CHUNK_SIZE_X; ++a)
    {
        for (short b = z; b < z +CHUNK_SIZE_Z; ++b)
        {
                    ...
                    m_TileManagerPtr->Get_Map_Height(a, b+1) <= LEVEL_WATER_TOP || m_TileManagerPtr->Get_Map_Height(a+1, b+1) <= LEVEL_WATER_TOP)
            {
                ...
                /// Position.
                /// ////////////////////////////////////////////////////////////////////
                m_TileManagerPtr->getMapPos(mapX, mapZ);
                mapX-=a;
                mapZ-=b;
                if ((mapX&1) != (mapZ&1))
                {
                ...
                pReal[o+10] = TILE_SIZE_X * (a+1-x);
                pReal[o+11] = q2;
                pReal[o+12] = TILE_SIZE_Z * (b-z);

                pReal[o+20] = TILE_SIZE_X * (a-x);
                pReal[o+21] = q2;
                pReal[o+22] = TILE_SIZE_Z * (b+1-z);
                // 2. Triangle
                pReal[o+30] = TILE_SIZE_X * (a-x);
                pReal[o+31] = q2;
                pReal[o+32] = TILE_SIZE_Z * (b+1-z);

                pReal[o+40] = TILE_SIZE_X * (a+1-x);
                pReal[o+41] = q2;
                pReal[o+42] = TILE_SIZE_Z * (b-z);

                pReal[o+50] = TILE_SIZE_X * (a+1-x);
                pReal[o+51] = q1;
                pReal[o+52] = TILE_SIZE_Z * (b+1-z);
                ...
                pReal[o+ 8] = a / 256.0;
                pReal[o+ 9] = b / 256.0;
                pReal[o+18] = (a + 1)/ 256.0;
                pReal[o+19] = b / 256.0;
                pReal[o+28] = a / 256.0;
                pReal[o+29] = (b + 1)/ 256.0;
                // 2. Triangle
                pReal[o+38] = a / 256.0;
                pReal[o+39] = (b + 1)/ 256.0;
                pReal[o+48] = (a + 1)/ 256.0;
                pReal[o+49] = b / 256.0;
                pReal[o+58] = (a + 1)/ 256.0;
                pReal[o+59] = (b + 1)/ 256.0;
#else
                /// ////////////////////////////////////////////////////////////////////
                /// Grid-Texture.
                /// ////////////////////////////////////////////////////////////////////
    ...
    for (int a = x; a < x+ CHUNK_SIZE_X; a += 1)
    {
        for (int b = z; b < z +CHUNK_SIZE_Z; b +=1 )
        {
                    ...
                    m_TileManagerPtr->Get_Map_Height(a, b+1) > LEVEL_WATER_TOP || m_TileManagerPtr->Get_Map_Height(a+1, b+1) > LEVEL_WATER_TOP)
            {
                ...
                /// Textures.
                /// ////////////////////////////////////////////////////////////////////
                col = (128.0 /1024.0) * m_TileManagerPtr->Get_Map_Texture_Col(a,b);
                row = (128.0 /1024.0) * m_TileManagerPtr->Get_Map_Texture_Row(a,b);

                int MIPMAP_SPACE = 1;
                col = (16 + 2 * MIPMAP_SPACE) /128.0 * m_TileManagerPtr->Get_Map_Texture_Col(a,b)+ MIPMAP_SPACE/128.0;
                row = (16 + 2 * MIPMAP_SPACE) /128.0 * m_TileManagerPtr->Get_Map_Texture_Row(a,b)+ MIPMAP_SPACE/128.0;

                // 1. triangle
                pReal1[o+ 6] = col;
                pReal1[o+ 7] = row;

                pReal1[o+14] = col + 1.0 / 8.0;
                pReal1[o+15] = row ;

                pReal1[o+22] = col;
                pReal1[o+23] = row + 1.0 / 8.0;
                // 2. Triangle
                pReal1[o+30] = col;
                pReal1[o+31] = row + 1.0 / 8.0;

                pReal1[o+38] = col + 1.0 / 8.0;
                pReal1[o+39] = row;

                pReal1[o+46] = col + 1.0 / 8.0;
                pReal1[o+47] = row + 1.0 / 8.0;
                o += 48;
            }
        }
    }
    ...
    for (short a = x; a < x+ CHUNK_SIZE_X; ++a)
    {
        for (short b = z; b < z +CHUNK_SIZE_Z; ++b)
        {
            ...
            //                    m_TileManagerPtr->Get_Map_Height(a, b+1) > LEVEL_WATER_TOP || m_TileManagerPtr->Get_Map_Height(a+1, b+1) > LEVEL_WATER_TOP)
            {
                ...
                ///   144 = 128 pixel texture + 16 pixel mipmapping spaces.
                col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a,b)+ MIPMAP_SPACE/1024;
                row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(a,b)+ MIPMAP_SPACE/1024;
                if ((mapX&1))
                    col+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                if ((mapZ&1))
                    row+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                // 1. Triangle
                pReal1[o+  6] = col;
                pReal1[o+  7] = row;
                pReal1[o+ 18] = col;
                pReal1[o+ 19] = row + 0.5 / 8.0;
                pReal1[o+ 30] = col + 0.5 / 16.0;
                pReal1[o+ 31] = row + 0.5 / 16.0;
                // 2. Triangle
                pReal1[o+ 42] = col;
                pReal1[o+ 43] = row + 0.5 / 8.0;
                pReal1[o+ 54] = col + 0.5 / 8.0;
                pReal1[o+ 55] = row + 0.5 / 8.0;
                pReal1[o+ 66] = col + 0.5 / 16.0;
                pReal1[o+ 67] = row + 0.5 / 16.0;
                // 3. Triangle
                pReal1[o+ 78] = col + 0.5 / 8.0;
                pReal1[o+ 79] = row + 0.5 / 8.0;
                pReal1[o+ 90] = col + 0.5 / 8.0;
                pReal1[o+ 91] = row;
                pReal1[o+102] = col + 0.5 / 16.0;
                pReal1[o+103] = row + 0.5 / 16.0;
                // 4. Triangle
                pReal1[o+114] = col + 0.5 / 8.0;
                pReal1[o+115] = row;
                pReal1[o+126] = col;
                pReal1[o+127] = row;
                ...
                */
                ...
                if (a)
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a-1, b)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(a-1, b)+ MIPMAP_SPACE/1024;
                    if ((mapX&1))
                        col+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                    if ((mapZ&1))
                        row+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                ...
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(0, b)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(0, b)+ MIPMAP_SPACE/1024;
                    if ((mapZ&1))
                        row+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                }
                // 1. Triangle (left)
                ...
                pReal1[o+  9] = row;
                pReal1[o+ 20] = col;
                pReal1[o+ 21] = row + 0.5 / 8.0;
                pReal1[o+ 32] = col + 0.5 / 16.0;
                pReal1[o+ 33] = row + 0.5 / 16.0;

                // 2. Triangle
                if (b)
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a, b-1)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(a, b-1)+ MIPMAP_SPACE/1024;
                    if ((mapX&1))
                        col+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                    if ((mapZ&1))
                        row+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                ...
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a, 0)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(a, 0)+ MIPMAP_SPACE/1024;
                    if ((mapX&1))
                        col+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                }
                pReal1[o+ 44] = col;
                pReal1[o+ 45] = row + 0.5 / 8.0;
                pReal1[o+ 56] = col + 0.5 / 8.0;
                pReal1[o+ 57] = row + 0.5 / 8.0;
                pReal1[o+ 68] = col + 0.5 / 16.0;
                pReal1[o+ 69] = row + 0.5 / 16.0;
                // 3. Triangle
                if (a < TILES_SUM_X)
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a+1, b)+ MIPMAP_SPACE/1024;
                ...
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(TILES_SUM_X, b)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(TILES_SUM_X, b)+ MIPMAP_SPACE/1024;
                    if ((mapZ&1))
                        row+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                }
                pReal1[o+ 80] = col + 0.5 / 8.0;
                pReal1[o+ 81] = row + 0.5 / 8.0;
                pReal1[o+ 92] = col + 0.5 / 8.0;
                pReal1[o+ 93] = row;
                pReal1[o+104] = col + 0.5 / 16.0;
                pReal1[o+105] = row + 0.5 / 16.0;
                // 4. Triangle
                if (b < TILES_SUM_Z)
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a, b+1)+ MIPMAP_SPACE/1024;
                ...
                {
                    col = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Col(a, TILES_SUM_Z)+ MIPMAP_SPACE/1024;
                    row = (128 + 4 * MIPMAP_SPACE) /1024.0 * m_TileManagerPtr->Get_Map_Texture_Row(a, TILES_SUM_Z)+ MIPMAP_SPACE/1024;
                    if ((mapX&1))
                        col+= (64.0 + 2 * MIPMAP_SPACE) / 1024.0;
                }
                pReal1[o+116] = col + 0.5 / 8.0;
                pReal1[o+117] = row;
                pReal1[o+128] = col;
                pReal1[o+129] = row;

...
{
  if (gpMainWin->selRenameAnim->value() > gpMainWin->selRenameAnim->size())
  {
    return;
  }
  if (gpMainWin->selRenameAnim->size() == 0)
  {
    return;
  }
  if (!fl_ask("Do you want to write the new Animation name?"))
  {
    return;
  }
...
bool Skeleton::Init()
{
  ...
  gpMainWin->butSaveAnim  ->deactivate();
  return true;
}
...
{
  if (gpMainWin->selAnimName->size() == 0)
  {
    return;
  }
...
{
  if (gpMainWin->selAnimName->value() > gpMainWin->selAnimName->size())
  {
    return;
  }
  if (gpMainWin->selAnimName->size() == 0)
  ...
  if (!fl_ask("Do you want to write the new Animation length?"))
  {
    return;
  }
...
void Skeleton::save(const char *filename)
{
  ...
  for(unsigned int i = 0; i < vecSkelXML.size(); ++i)
  {
    outSkel << vecSkelXML[i] << endl;
  }
...
void Skeleton::clearAnimations()
{
  ...
  for (unsigned int i = 0; i < vecAnimation.size(); i++)
  {
    delete vecAnimation[i];
  }
  vecAnimation.clear();
  vecSkelXML.clear();
  gpMainWin->selAnimName->redraw();
}
...
void Skeleton::load(const char *filename)
{
  ...
  while(getline(inSkeleton, line))
  {
    ...
    /////////////////////////////////////////////////////////////////////////
    if (line.find("animation>")     != string::npos)
    {
      sectAnim = false;
    }
  ...
  fillAnimSelectCombo();
  selAnimation(0);
  gpMainWin->butSaveAnim  ->activate();
}
...
void Skeleton::scale(double scale)
{
  ...
  int start,stop;
  char buffer[100];
  if (!vecSkelXML.size()) return;
  for(unsigned int i = 0; i < vecSkelXML.size(); ++i)
  {
    if (vecSkelXML[i].find("<position ") != string::npos)
    {
      // Found a position data.
      start = vecSkelXML[i].find("\"");
      stop  = vecSkelXML[i].find("\"",++start);
      strTmp = vecSkelXML[i].substr(start, stop-start);
      x = atof (strTmp.c_str()) * scale;
      start = vecSkelXML[i].find("\"", ++stop);
      stop  = vecSkelXML[i].find("\"",++start);
      strTmp = vecSkelXML[i].substr(start, stop-start);
      y = atof (strTmp.c_str()) * scale;
      start = vecSkelXML[i].find("\"", ++stop);
      stop  = vecSkelXML[i].find("\"",++start);
      strTmp = vecSkelXML[i].substr(start, stop-start);
      z = atof (strTmp.c_str()) * scale;
      //LogFile::getSingleton().Info("x, y, z: %f , %f, %f\n", x,y,z);
      sprintf(buffer, "                        <position x=\"%f\" y=\"%f\" z=\"%f\" />", x,y,z);
      vecSkelXML[i] = buffer;
    }
  }
  save("test_skel.xml");

...
void Material::renameMaterial()
{
    string line, newName;
    unsigned int start;
    if (gpMainWin->selRenameMaterial->value() > gpMainWin->selRenameMaterial->size()) { return; }
    if (gpMainWin->selRenameMaterial->size() == 0) { return; }
//    if (!fl_ask("Do you want to write the new Material name?")) { return; }
    ...
    start = newName.find(".");
    if (start != string::npos) { newName.replace(start, 1, "/"); }
...
{
    if (gpMainWin->selMaterialName->size() == 0) { return; }
...
// ========================================================================
bool Material::parseAnimName(const string &name)
{
...
void Material::fillMatSelectCombo()
{
    ...
    { 
        ...
        start = name.find("/");
        if (start != string::npos) { name.replace(start, 1, "."); }

...
bool Mesh::Init()
{
  ...
  gpMainWin->butScaleModel    ->deactivate();
  return true;
}
// ========================================================================
//
...
void Mesh::save(const char *filename)
{
  ...
  for(unsigned int i = 0; i < vecMeshXML.size(); ++i)
  {
    outSkel << vecMeshXML[i] << endl;
  }
...
void Mesh::load(const char *filename)
{
  ...
  vecMeshXML.clear();
  Material::getSingleton().clearMaterials();
  // Read in MeshFile.
  ...
  gpMainWin->butScaleModel->activate();
}
...
void Mesh::scale(double scale)
{
  ...
  int start,stop;
  char buffer[100];
  if (!vecMeshXML.size()) return;
  for(unsigned int i = 0; i < vecMeshXML.size(); ++i)
  {
    if (vecMeshXML[i].find("<position ") != string::npos)
    {
      // Found a position data.
      start = vecMeshXML[i].find("\"");
      stop  = vecMeshXML[i].find("\"",++start);
      strTmp = vecMeshXML[i].substr(start, stop-start);
      x = atof (strTmp.c_str()) * scale;
      start = vecMeshXML[i].find("\"", ++stop);
      stop  = vecMeshXML[i].find("\"",++start);
      strTmp = vecMeshXML[i].substr(start, stop-start);

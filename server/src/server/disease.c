...
int move_symptom(object *symptom) {
  ...
  else hit_player(victim,(int)MAX((float)1,(float)-victim->stats.maxhp * (float)symptom->stats.dam / (float)100.0),symptom,symptom->attacktype);
  if(symptom->stats.maxsp>0) sp_reduce = symptom->stats.maxsp;
  else sp_reduce = (int)MAX((float)1,(float)victim->stats.maxsp * (float)symptom->stats.maxsp/(float)100.0);

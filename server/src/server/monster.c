...
static inline void cleanup_behaviour_parameters(
        struct mob_behaviour *behaviour)
{
    ...
    if(behaviour->parameters) {
        ...
        return_poolarray(behaviour->parameters, 
                behaviour->declaration->nrof_params,
...
struct mob_behaviour *init_behaviour(behaviourclass_t classid, int behaviourid)
{
    ...
    if(behaviour->declaration->nrof_params > 0) {
        int i;
        behaviour->parameters = get_poolarray(POOL_BEHAVIOUR_PARAM, 
                behaviour->declaration->nrof_params);
        for(i=0; i< behaviour->declaration->nrof_params; i++) {
...
//    LOG(llevDebug,"Generating behaviourset for %s\n", STRING_OBJ_NAME(op));
    ...
    /* Processes */
    last = set->behaviours[BEHAVIOURCLASS_PROCESSES] = init_behaviour(
            BEHAVIOURCLASS_PROCESSES, AIBEHAVIOUR_LOOK_FOR_OTHER_MOBS);
    last = last->next = init_behaviour(
            BEHAVIOURCLASS_PROCESSES, AIBEHAVIOUR_FRIENDSHIP);
    if(! QUERY_FLAG(op, FLAG_NO_ATTACK)) 
        last = last->next = init_behaviour(
                BEHAVIOURCLASS_PROCESSES, AIBEHAVIOUR_CHOOSE_ENEMY);

    /* Moves */
    if(QUERY_FLAG(op, FLAG_STAND_STILL)) 
        last = set->behaviours[BEHAVIOURCLASS_MOVES] = init_behaviour(
                BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_STAND_STILL);
    else {
        last = set->behaviours[BEHAVIOURCLASS_MOVES] = init_behaviour(
                BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_SLEEP);
        if(op->run_away) 
        {
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_RUN_AWAY_FROM_ENEMY);
        ...
        if(! QUERY_FLAG(op, FLAG_NO_ATTACK)) {
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_MOVE_TOWARDS_ENEMY);
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_MOVE_TOWARDS_ENEMY_LAST_KNOWN_POS);
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_SEARCH_FOR_LOST_ENEMY);
        }
        if((op->move_type & HI4) == WPOINT)
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_MOVE_TOWARDS_WAYPOINT);
        if(QUERY_FLAG(op, FLAG_RANDOM_MOVE)) {
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_MOVE_RANDOMLY);
            if(op->item_race != 255) {
        ...
        last = last->next = init_behaviour(
                BEHAVIOURCLASS_MOVES, AIBEHAVIOUR_MOVE_TOWARDS_HOME);
    }
    
    /* Actions */
    if(! QUERY_FLAG(op, FLAG_NO_ATTACK)) {
        last = set->behaviours[BEHAVIOURCLASS_ACTIONS] = init_behaviour(
                BEHAVIOURCLASS_ACTIONS, AIBEHAVIOUR_MELEE_ATTACK_ENEMY);
        if(QUERY_FLAG(op, FLAG_CAST_SPELL))
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_ACTIONS, AIBEHAVIOUR_SPELL_ATTACK_ENEMY);
        if(QUERY_FLAG(op, FLAG_READY_BOW))
            last = last->next = init_behaviour(
                    BEHAVIOURCLASS_ACTIONS, AIBEHAVIOUR_BOW_ATTACK_ENEMY);
    }
...
//                    LOG(llevDebug,"    behaviour %s\n", buf);
            ...
            if(new_behaviour == NULL) {
                LOG(llevBug,"BUG: unknown %s behaviour %s of %s\n", 
                        behaviourclasses[class].name, buf, STRING_OBJ_NAME(op));
                continue;
            } 
...
void ai_run_away_from_enemy(object *op, struct mob_behaviour_param *params, move_response *response)
{
    ...
    if(QUERY_FLAG(op, FLAG_SCARED) && op->enemy) 
    {
        ...
        if(op->stats.maxhp && AIPARAM_PRESENT(AIPARAM_RUN_AWAY_FROM_ENEMY_HP_THRESHOLD))
        {
            ...
            if(op->stats.hp == op->stats.maxhp || (op->stats.hp*100)/op->stats.maxhp > 
                    AIPARAM_INT(AIPARAM_RUN_AWAY_FROM_ENEMY_HP_THRESHOLD) * 2)
            {
        ...
        /* Become scared? */
        if(op->stats.maxhp && AIPARAM_PRESENT(AIPARAM_RUN_AWAY_FROM_ENEMY_HP_THRESHOLD)
                && (op->stats.hp*100)/op->stats.maxhp < 
...
int move_monster(object *op) {
    ...
    {
        ((void(*)(object *, struct mob_behaviour_param *))behaviour->declaration->func)
            ...
            behaviour != NULL; behaviour = behaviour->next) 
    {
        ((void(*)(object *, struct mob_behaviour_param *, move_response *))
         behaviour->declaration->func)(op, behaviour->parameters, &response);
            ...
            behaviour != NULL; behaviour = behaviour->next) 
    {
        if (((int(*)(object *, struct mob_behaviour_param *))
                    behaviour->declaration->func)(op, behaviour->parameters))
            break;
    }

...
static int attempt_jump (object *pl, int dir, int spaces) {
	...
	for(tmp=get_map_ob(m,xt,yt); tmp;tmp=tmp->above) { 
	    if(wall(tmp->map,xt,yt)) {           /* Jump into wall*/ 
...
int do_skill_ident2(object *tmp,object *pl, int obj_class) 
{
  ...
  int skill_value = SK_level(pl) + get_weighted_skill_stats(pl);
	...
	if(!QUERY_FLAG(tmp,FLAG_IDENTIFIED) && !QUERY_FLAG(tmp,FLAG_NO_SKILL_IDENT) 
	   && need_identify(tmp) 
	   && !IS_SYS_INVISIBLE(tmp) && tmp->type==obj_class) { 
...
int use_oratory(object *pl, int dir) {
    ...
    if(!(m=out_of_map(pl->map,&x,&y))) return 0;
...
int write_scroll (object *pl, object *scroll) {
    ...
    if(!(spells[chosen_spell].spell_use&SPELL_USE_SCROLL)) { /* Tried to write non-scroll spell */
...
object *find_throw_ob( object *op, char *request ) {
  ...
  for(tmp=op->inv;tmp;tmp=tmp->below) {
      ...
      if(IS_SYS_INVISIBLE(tmp)||QUERY_FLAG(tmp,FLAG_INV_LOCKED)) continue;
    ...
    if (QUERY_FLAG(tmp,FLAG_APPLIED)) {
      if(tmp->type!=WEAPON) {
        new_draw_info_format(NDI_UNIQUE, 0,op,
          "You can't throw %s.",query_base_name(tmp));
        tmp = NULL;
      } else if (QUERY_FLAG(tmp,FLAG_CURSED)||QUERY_FLAG(tmp,FLAG_DAMNED)) {
        new_draw_info_format(NDI_UNIQUE, 0,op,
          "The %s sticks to your hand!",query_base_name(tmp));
        tmp = NULL;
      } else {
...
void do_throw(object *op, object *toss_item, int dir) {
	...
	*/
	load_factor=1.0f; /* we need something more clever here, using weight_limit */
...
/*    LOG(llevDebug," item %s weight= %d (dam:%d str_factor:%f bonus: %d)\n",throw_ob->name,throw_ob->weight, dam, str_factor, dam_bonus[eff_str]);*/
	...
	/*
    throw_ob->stats.wc = 22 - dex_bonus[op->stats.Dex?dex_bonus[op->stats.Dex]:0]
	...
	*/
	...
	if(throw_ob->material&M_PAPER||throw_ob->material&M_CLOTH) {
	    throw_ob->stats.dam -= 5; throw_ob->speed *= 0.8f;
	    throw_ob->stats.wc += 3; throw_ob->stats.food -= 30;
	}
	/* light obj have more wind resistance, fly slower*/
	if(throw_ob->weight>500) throw_ob->speed *= 0.8f;
	if(throw_ob->weight>50) throw_ob->speed *= 0.5f;

...
void polymorph_living(object *op) {
	...
	create_treasure(op->randomitems,op,GT_INVISIBLE,map->difficulty,T_STYLE_UNSET,ART_CHANCE_UNSET,0);
...
{
    /* Not unique */
    new_draw_info_format(NDI_WHITE, 0, who,
...
void polymorph_item(object *who, object *op) {
     ...
     */
    for(at = first_archetype ; at != NULL; at = at->next) {
	if(at->clone.type == op->type && !IS_SYS_INVISIBLE(&at->clone) && 
...
int cast_wor(object *op, object *caster) {
  ...
  dummy->speed = 0.002f * ((float)(spells[SP_WOR].bdur
...
*/
	...
	case SP_EARTH_WALL:
		...
		tmp->resist[i]=0;
	    tmp->stats.hp = spells[spell_type].bdur +
	...
	case SP_DARKNESS:
	    ...
	    tmp->stats.food = spells[SP_DARKNESS].bdur
...
int dimension_door(object *op,int dir) {
	 ...
	 */
	for(;dist>0&&blocked(op, op->map,op->x+freearr_x[dir]*dist,
                                 op->y+freearr_y[dir]*dist,op->terrain_flag);dist--);
	...
	switch(spell_type)
	{
		...
		case SP_CURE_POISON: 
			...
			if(op != target && op->type == PLAYER)
				new_draw_info_format(NDI_UNIQUE, 0,op, "You casts cure poison on %s!", target->name?target->name:"someone");
			...
			if(success)
			{
				...
				if(op != target && op->type == PLAYER)
					new_draw_info_format(NDI_UNIQUE, 0,op, "%s body seems cleansed.", target->name?target->name:"someone");
			}
			else
			{
				if(target->type == PLAYER)
		...
		case SP_CURE_CONFUSION: 
			...
			if(op != target && op->type == PLAYER)
				new_draw_info_format(NDI_UNIQUE, 0,op, "You casts cure confusion on %s!", target->name?target->name:"someone");
			...
			if(success)
			{
				...
				if(op != target && op->type == PLAYER)
					new_draw_info_format(NDI_UNIQUE, 0,op, "%s mind seems clearer.", target->name?target->name:"someone");
			}
			else
			{
				if(target->type == PLAYER)
		...
		case SP_CURE_BLINDNESS: 
			...
			if(op != target && op->type == PLAYER)
				new_draw_info_format(NDI_UNIQUE, 0,op, "You casts cure blindness on %s!", target->name?target->name:"someone");
			...
			if(success)
			{
				...
				if(op != target && op->type == PLAYER)
					new_draw_info_format(NDI_UNIQUE, 0,op, "%s vision seems to return.", target->name?target->name:"someone");
			}
			else
			{
				if(target->type == PLAYER)
...
int cast_create_missile(object *op, object *caster,int dir, char *stringarg)
{
  ...
  if (!stringarg || ((1 + SP_level_strength_adjust(op, caster,SP_CREATE_MISSILE)) -
                     (3 * missile_plus)) < 0)
    missile_plus = spells[SP_CREATE_MISSILE].bdam +
  ...
  missile->nrof = spells[SP_CREATE_MISSILE].bdur
...
int remove_curse(object *op, object *target, int type, SpellTypeFrom src) 
{
	...
	{
		if(op->type == PLAYER)
			new_draw_info_format(NDI_UNIQUE, 0,op, "You cast remove %s on %s.",type==SP_REMOVE_CURSE?"curse":"damnation",query_base_name(target));
		else if(target->type == PLAYER)
			new_draw_info_format(NDI_UNIQUE, 0,target, "%s cast remove %s on you.",query_base_name(op),type==SP_REMOVE_CURSE?"curse":"damnation");
	}

	/* Player remove xx only removes applied stuff, npc remove clears ALL */
	for (tmp = target->inv; tmp; tmp = tmp->below)
	{
		if ((src ==spellNPC || QUERY_FLAG(tmp, FLAG_APPLIED)) && (QUERY_FLAG(tmp, FLAG_CURSED) ||
							(type == SP_REMOVE_DAMNATION && QUERY_FLAG(tmp, FLAG_DAMNED))))
		{
			...
			{
				if(target->type == PLAYER)
					new_draw_info_format(NDI_UNIQUE, 0,target, "The %s curse is stronger as the prayer!", query_base_name(tmp));
...
int cast_identify(object *op, int level, object *single_ob, int mode) 
{
	...
	for (tmp = op->inv; tmp ; tmp = tmp->below)
	{
		...
		if(!QUERY_FLAG(tmp, FLAG_IDENTIFIED) && !IS_SYS_INVISIBLE(tmp) && need_identify(tmp))
		{
			...
			{
				if (op->type==PLAYER) 
					new_draw_info_format(NDI_UNIQUE, 0, op, "The %s is too powerful for this identify!", query_base_name(tmp));
			}
			else
			{
				identify(tmp);
...
/*  see if we can find someone to give sp to.  */
	...
	int sp=(plyr->stats.sp + spells[SP_TRANSFER].bdam
...
int summon_cult_monsters(object *op, int old_dir) {
	...
	dir = absdir(dir + 1);
	if (arch_blocked(mon->arch,mon,op->map, op->x + freearr_x[dir],
		op->y + freearr_y[dir])) {
...
int finger_of_death(object *op, object *caster, int dir) {
  ...
  hitter->level = path_level_mod (caster, spells[SP_FINGER_DEATH].bdam + 
...
int animate_weapon(object *op,object *caster,int dir, archetype *at, int spellnum) {
  ...
  if(tmp->speed > 3.33f) tmp->speed = 3.33f;
...
int cast_faery_fire(object *op,object *caster) {
  ...
  factor=spells[SP_FAERY_FIRE].bdur 
	+ SP_level_strength_adjust(op,caster,SP_FAERY_FIRE);
  r=spells[SP_FAERY_FIRE].bdam

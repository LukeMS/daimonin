	...
	int dir, archetype *at, int type, int magic)
{
  ...
  case SP_DIVINE_SHOCK:
    case SP_BALL_LIGHTNING:
	tmp->stats.food=spells[type].bdur +
...
int can_see_monsterP(mapstruct *m, int x, int y,int dir) {
    ...
    if (!(m=out_of_map(m, &dx, &dy))) return 0;
...
static int SP_level_gracepoint_cost(object *op, object *caster, int spell_type)
{
  ...
  int grace;
  if(spells[spell_type].spl)
   grace= (int) (spells[spell_type].sp * 
	       (1.0 + 
	(MAX(0,	(float)(level-spells[spell_type].level)/
	(float)spells[spell_type].spl ))));
  else grace= spells[spell_type].sp;
		 ...
		 int magic) 
{
  ...
  new_aura->stats.food = spells[spell_type].bdur +
                  10* SP_level_strength_adjust(op,caster,spell_type);
  new_aura->stats.dam = spells[spell_type].bdam
...
void put_a_monster(object *op,const char *monstername) {
  ...
  if(dir!=-1) {
    ...
    if (head->randomitems) 
		create_treasure_list(head->randomitems, head, GT_INVISIBLE, op->level?op->level:op->map->difficulty,T_STYLE_UNSET,ART_CHANCE_UNSET,0,NULL);
...
int cast_smite_spell (object *op, object *caster,int dir, int type) {
  ...
  /* tailor the effect by priest level and worshipped God */
   ...
   effect->stats.hp=spells[type].bdur +
                SP_level_strength_adjust(op,caster,type);
   /* how much woe to inflict :) */
   effect->stats.dam=spells[type].bdam +

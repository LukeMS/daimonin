...
static void load_srv_files(char *fname, int id, int cmd)
{
    ...
    if ((int) numread < flen)
    {
		...
		*((uint32 *)(comp_tmp+1))=(uint32)(flen); //we send as uint32 the uncompressed filesize
		SrvClientFiles[id].sockbuf = SOCKBUF_COMPOSE( SERVER_CMD_DATA, NULL, (char *)comp_tmp, numread+5, SOCKBUF_FLAG_STATIC);
    ...
    else
    {
		...
		*file_tmp = (char) cmd;
		SrvClientFiles[id].sockbuf = SOCKBUF_COMPOSE(SERVER_CMD_DATA, NULL, (char *)file_tmp, flen+1, SOCKBUF_FLAG_STATIC);

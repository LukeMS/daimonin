                     ...
                     RMParms *RP)
{
      ...
      else */
    {
        ...
        if (the_chest->randomitems)
            unlink_treasurelists(the_chest->randomitems, FALSE);
        the_chest->randomitems = link_treasurelists("chest", OBJLNK_FLAG_REF);
        the_chest->stats.hp = n_treasures;
    }
...
object * find_monster_in_room_recursive(char **layout, mapstruct *map, int x, int y, RMParms *RP)
{
    ...
    {
        theMonsterToFind = find_monster_in_room_recursive(layout, map, x + freearr_x[i % 8 + 1],
                                                          y + freearr_y[i % 8 + 1], RP);
        if (theMonsterToFind != NULL)
            return theMonsterToFind;
    }
    return theMonsterToFind;
                                  ...
                                  RMParms *RP)
{
    ...
    else
    {
        ...
        for (i = RANDOM() % 8,j = 0; j < 8 && theMonsterToFind == NULL; i++,j++)
        {
            find_doors_in_room_recursive(layout, map, x + freearr_x[i % 8 + 1], y + freearr_y[i % 8 + 1], doorlist,
                                         ndoors, RP);
        }
    }
}

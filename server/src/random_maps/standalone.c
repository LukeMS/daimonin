    ...
    switch (op->type)
    {
        case SHOP_FLOOR:
          ...
          do
          {
              ...
              while ((tmp = generate_treasure(op->randomitems, op->map == NULL ? op->stats.exp : op->map->difficulty))
                  == NULL
                  && --i);
              if (tmp == NULL)
                  return 0;
        ...
        case TREASURE:
          create_treasure(op->randomitems, op, GT_ENVIRONMENT, op->map == NULL ? op->stats.exp : op->map->difficulty,

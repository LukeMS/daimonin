 ...
 * are always executed 
 */
BehaviourClass(PROCESSES, /** Simply look around for other mobs nearby */
...
Behaviour(MOVE_RANDOMLY, ai_move_randomly, /** Limits the x-distance from the starting point */
...
Behaviour(RUN_AWAY_FROM_ENEMY, ai_run_away_from_enemy, /** Become scared if current_hp < HP_THRESHOLD % of max_hp */
Parameter(RUN_AWAY_FROM_ENEMY, HP_THRESHOLD, INTEGER, OPTIONAL, 10))

Behaviour(KEEP_DISTANCE_TO_ENEMY, ai_keep_distance_to_enemy, Parameter(KEEP_DISTANCE_TO_ENEMY, MIN_DIST, INTEGER, OPTIONAL, 4)
Parameter(KEEP_DISTANCE_TO_ENEMY, MAX_DIST, INTEGER, OPTIONAL, 6))

Behaviour(STEP_BACK_AFTER_SWING, ai_step_back_after_swing, Parameter(STEP_BACK_AFTER_SWING, DIST, INTEGER, OPTIONAL, 3))
...
*/
BehaviourClass(ACTIONS, /** Attack the current enemy with melee weapons if within range */

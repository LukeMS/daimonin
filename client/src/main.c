 ...
 */
        ...
        if (map_transfer_flag)
                strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "Transfer Character to Map...", 300, 300, NDI_COLR_WHITE, NULL);
        ...
        if (GameStatus == GAME_STATUS_PLAY)
        {
                ...
                cpl.menustatus == MENU_NO)
            {
                ...
                strout_blt(ScreenSurface, &font_small, STROUT_LEFT, buf, rec.x, rec.y,
                           NDI_COLR_WHITE, NULL);
            }
...
static void ShowIntro(char *text, int progress)
{
    ...
    if (text)
    {
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, text, x+370, y+585, NDI_COLR_WHITE, NULL);
    }
    else
    {
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "** Press Key **", x+375, y+585, NDI_COLR_WHITE, NULL);
    }
...
static void FlipScreen(void)
{
    if (GameStatus < GAME_STATUS_WAITFORPLAY)
    {
        ...
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, buf, (Screensize.xoff/2)+10, (Screensize.yoff/2)+585, NDI_COLR_WHITE, NULL);
    }

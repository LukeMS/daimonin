...
void widget_show_group(int x, int y)
{
    ...
    strout_blt(ScreenSurface, &font_tiny, STROUT_LEFT, "Group:", x +50, y+2 , skin_prefs.widget_title, NULL);


    if (global_group_status < GROUP_INVITE)
    {
        strout_blt(ScreenSurface, &font_tiny, STROUT_LEFT, "type '/help group' for info", x+13, y+13, skin_prefs.widget_info, NULL);
        return;
    }
    ...
    if (global_group_status == GROUP_INVITE || global_group_status == GROUP_WAIT)
    {
        ...
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "GROUP INVITE", x+30, y+13,skin_prefs.widget_key, NULL);
        len = strout_width(&font_small, group_invite);
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, group_invite, x + 60-len/2, y + 45, skin_prefs.pname_other, NULL);
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "has invited you", x + 28, y +65, skin_prefs.widget_info, NULL);
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "to join a group.", x + 28, y +78, skin_prefs.widget_info, NULL);
    ...
    else /* status: GROUP_MEMBER */
    {
        ...
        for (s = 0; s < GROUP_MAX_MEMBER; s++)
        {
            ...
            if (group[s].name[0] != '\0')
            {
                ...
                sprite_blt(skin_sprites[SKIN_SPRITE_GROUP], x + group_pos[s][0] + 2, y + group_pos[s][1] + 1, NULL, NULL);
                strout_blt(ScreenSurface, &font_small, STROUT_LEFT, group[s].name, x + group_pos[s][0] + 33, y + group_pos[s][1] + 1, colr, NULL);
                sprintf(buf, "%3d", group[s].level);
                strout_blt(ScreenSurface, &font_tiny, STROUT_LEFT, buf, x + group_pos[s][0] + 8, y + group_pos[s][1], skin_prefs.widget_valueEq, NULL);
...
void show_group(int x, int y)
{
    ...
    */
    ...
    if (global_group_status < GROUP_INVITE)
    {
        strout_blt(ScreenSurface, &font_tiny, STROUT_LEFT, "type '/help group' for info", 40, Screensize.yoff+585, skin_prefs.widget_info, NULL);
        return;
    }
    ...
    if (global_group_status == GROUP_INVITE || global_group_status == GROUP_WAIT)
    {
        ...
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, "GROUP INVITE", x + group_pos[0][0] + 76, y + group_pos[0][1] + 5,skin_prefs.widget_key, NULL);
        len = strout_width(&font_small, group_invite);
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, group_invite, x + group_pos[0][0]+107-len/2, y + group_pos[0][1] + 19,skin_prefs.pname_other, NULL);
        strout_blt(ScreenSurface, &font_small, STROUT_LEFT, " has invited you to join a group.", x + group_pos[0][0] + 40, y + group_pos[0][1] + 31,skin_prefs.widget_info, NULL);
